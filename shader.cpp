...
bool Shader_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	buffer_desc.ByteWidth = sizeof(XMFLOAT4X4)*2; // バッファのサイズ constant//MATRIX
	...
	sampler_desc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP; // U座標のアドレス(モード)
	sampler_desc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP; // V座標のアドレス(モード)
	sampler_desc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP; // W座標のアドレス(モード) 変更すると面白いことができる
	sampler_desc.MipLODBias = 0;
	sampler_desc.MaxAnisotropy = 16; // 最大異方性フィルタリングレベル
	sampler_desc.ComparisonFunc = D3D11_COMPARISON_ALWAYS; // 比較関数
	sampler_desc.MinLOD = 0; // ボーダーカラーのR成分
	sampler_desc.MaxLOD = D3D11_FLOAT32_MAX; // ボーダーカラーのG成分

	ID3D11SamplerState* samplerState;
	g_pDevice->CreateSamplerState(&sampler_desc, &g_SamplerState);

	g_pContext->PSSetSamplers(0, 1, &g_SamplerState); // ピクセルシェーダーにサンプラーステートを設定
...
void Shader_SetMatrix(const MATRIX& matrix)
{
	...
	XMStoreFloat4x4(&transpose[0], XMMatrixTranspose(matrix.Mtx));//Matrix
